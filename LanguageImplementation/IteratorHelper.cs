...
namespace LanguageImplementation
{
    ...
    /// <summary>
    /// The iterator generated by iterator. It produces tuples with the first index being the current index into the iterator and the
    /// second index is the current iterated item.
    /// </summary>
    public class EnumerateIterator : PyIterable
    {
        private int currentIndex;
        private PyIterable itr;

        public EnumerateIterator(PyIterable itr)
        {
            currentIndex = 0;
            this.itr = itr;
        }
        ...
        {
            var next_item = await itr.Next(interpreter, context, itr);
            if(context.CurrentException is StopIteration)
            {
                return null;
            }

            var returnTuple = PyTuple.Create(new object[] { PyInteger.Create(currentIndex), next_item });
            currentIndex += 1;
            return returnTuple;
        }
    }
