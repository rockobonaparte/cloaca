...
namespace CloacaTests
{
    ...
    public class EmbeddingTests : RunCodeTest
    {
        ...
        {
            // BOOKMARK: Start trying to work with an IPyCallable directly in CALL_FUNCTION in the interpreter.
            // Review if I should use a wrapped code object in some new way first.
            // If you stick with WrappedCodeObject, then try to make it easier to use (and revise MockBlockedReturnValue so that the wrapped method body is properly capitalized and looks like it belonged this way).
            var blockedReturnMock = new MockBlockedReturnValue();

            // Looks like we can't respond to the task generated by the wrapped code. :(

            //Expression<Action<PyTypeObject>> blockedReturn_expr = instance => blockedReturnMock.wrappedMethodBody(null);
            //var blockedReturn_methodInfo = ((MethodCallExpression)blockedReturn_expr.Body).Method;
            //var blockedReturn_wrapper = new WrappedCodeObject("blocking_call", blockedReturn_methodInfo, blockedReturnMock);

            runBasicTest("a = blocking_call()\n", new Dictionary<string, object>()
            {
                { "blocking_call", blockedReturnMock }
//                { "blocking_call", blockedReturn_wrapper }
